...
describe('Vector', () => {
    ...
    test('crossProduct XY -> Z', () =>
        expect(Vector.cross_product(Vector.FORWARD, Vector.RIGHT)).toEqual(Vector.UP))

    test('crossProduct YZ -> X', () =>
        expect(Vector.cross_product(Vector.RIGHT, Vector.UP)).toEqual(Vector.FORWARD))

    test('crossProduct ZX -> Y', () =>
        expect(Vector.cross_product(Vector.UP, Vector.FORWARD)).toEqual(Vector.RIGHT))

    test('fuzz cross product produces perpenticular vector', () => {
        const generator = new RandomVectorGenerator()
        for (let i = 0; i < 10; i++) {
            const a = generator.next_vector_from_unit_sphere_volume()
            const b = generator.next_vector_from_unit_sphere_volume()
            const c = Vector.cross_product(a,b)
            expect(a.dot(c)).toBeCloseTo(0, 10) // should be perpenticual
            expect(b.dot(c)).toBeCloseTo(0, 10) // should be perpenticual
        }
    })

    test('cross product should make perpenticular vector', () => {
    ...
    test('with_positive_zeroes', () => {
        expect(new Vector(-0,-0,-0).with_positive_zeroes()).toEqual(new Vector(0,0,0))
    })
})
