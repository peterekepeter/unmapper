...
export class SideRenderTransform implements RenderTransform {
    ...
    view_transform_x(vector: Vector): number {
        return (vector.x - this.view_center.x) * this.deviceSize * this.scale + this.width / 2;
    }

    view_transform_y(vector: Vector): number {
        return (vector.z - this.view_center.z) * -1 * this.deviceSize * this.scale + this.height / 2;
    }
    ...
    canvas_to_world_location(canvas_x: number, canvas_y: number): Vector {
        return new Vector(
            this.view_center.x + (canvas_x - this.width/2) / this.deviceSize / this.scale,
            this.view_center.y,
            this.view_center.z - (canvas_y - this.height/2) / this.deviceSize / this.scale)
    }
}
